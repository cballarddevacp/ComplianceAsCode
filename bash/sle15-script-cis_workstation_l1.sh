...
# $ oscap xccdf generate fix --profile xccdf_org.ssgproject.content_profile_cis_workstation_l1 --fix-type bash ssg-sle15-ds.xml
#
...
find /etc/issue.d/  -type f ! -group 0 -regex '^.*$' -exec chgrp 0 {} \;
...
###############################################################################
# BEGIN fix (82 / 296) for 'xccdf_org.ssgproject.content_rule_package_pam_apparmor_installed'
###############################################################################
(>&2 echo "Remediating rule 82/296: 'xccdf_org.ssgproject.content_rule_package_pam_apparmor_installed'")
# Remediation is applicable only in certain platforms
if [ ! -f /.dockerenv ] && [ ! -f /run/.containerenv ]; then
